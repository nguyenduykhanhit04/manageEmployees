# Tài liệu Thiết kế API — Update Employee

| Thông tin | Giá trị |
|---|---|
| Tên system | TKCB |
| Loại system | Thiết kế API |
| Category chức năng | Update Employee |
| Người tạo | ThanhPD |
| Ngày tạo | 2023-01-04 |
...
| &nbsp;&nbsp;— | key/value trong object | | string | `{}` | Tên hạng mục / giá trị hạng mục | Lưu vào table `employees_certifications` |

> Ghi chú: Chỉ `certifications` có đánh dấu "Bắt buộc" = "-" (không bắt buộc) trong bảng gốc; các quy tắc bắt buộc chi tiết theo từng field cụ thể được nêu ở mục "5. Chi tiết xử lý — 1. Validate parameter" bên dưới. Khác với API **Add Employee**, ở đây `certifications` là **object** đơn (chỉ 1 chứng chỉ) chứ không phải mảng.
...
2. update nhân viên
  ...
  │
  ▼
3. Update thông tin chứng chỉ tiếng Nhật của nhân viên
